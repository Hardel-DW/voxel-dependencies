...
export function getRegistry<T>(
	...
	for (const file of Object.keys(files)) {
		...
		const compressedPath = file
			...
			.slice(2)
			.map((part, index, arr) =>
				index === arr.length - 1 ? part.split(".")[0] : part,
			)
			.join("/");
		const resource = compressedPath.startsWith(registry)
			? compressedPath.slice(registry.length + 1)
			: null;
...
export function readDatapackFile<T>(
	datapack: Record<string, Uint8Array>,
	identifier: Identifier,
): T | undefined {
